...
def fetch_object(curr_api_url, curr_oi, batch_num, row):
    try:
        ...
        try:
            ...
            if curr_image_url not in ("none", ""):
                supabase.table("art_ifacts").update({
            ...
            with open("bad_apples.txt", "a") as f:
                f.write(f"{curr_oi}, failed to decode JSON\n")
            return "continue"

    except Exception as e:
        with open("bad_apples.txt", "a") as f:
            f.write(f"{curr_oi}, failed because of error: {e}\n")
...
def preload_image_urls():
    ...
    for batch_num in range(num_batches):
        ...
        response = supabase.table("art_ifacts").select("*").range(start, end).is_("has_image", "null").execute()
        batch = response.data
