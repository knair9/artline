...
def get_artifacts(
    ...
    museum_lower = museum.lower().strip() if museum else None
    ...
    # Route to museum sources safely
    try:
        ...
        print("ERROR inside museum selection:", e)
        raise HTTPException(status_code=500, detail=str(e))

    if artifacts is None:
        raise HTTPException(status_code=500, detail="Museum scraper returned no data")

    print(f"Fetched {len(artifacts)} artifacts between {start} and {end}")
    return artifacts
